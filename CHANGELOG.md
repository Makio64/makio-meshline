...
- Change `smoothSharpBends` to default to `false`, preserving exact input-to-GPU topology for dynamic `setPositions()` updates, custom per-vertex attributes, and framework wrappers. Enable `.smoothSharpBends( true )` explicitly for static sharp polylines that benefit from CPU-side corner subdivision.
