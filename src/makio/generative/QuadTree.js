import { randomSylable } from '../utils/hangul/HangulUtils'
import { options } from '../utils/input/wheel'
...
export default class QuadTree {
	...
	constructor( width, height, { total = -1, maxDepth = 4, minSize = -1, test = null, jitter = 0, randomNext = 0.6, sizes = { 0: 0, 1: 0 }, record = false } = {} ) {
		...
		this.cells.add( new Cell( 0, 0, width, height ) )
	...
	quadtree( iteration, cell, forced ) {
		...
		//top-left
		let c = new Cell( cell.x, cell.y, cell.width * p1, cell.height * p2 )
		c.time = this.time++
		this.newCells.add( c )

		//top-right
		c = new Cell( cell.x + cell.width * p1, cell.y, cell.width * ( 1 - p1 ), cell.height * p2 )
		c.time = this.time++
		this.newCells.add( c )

		// bottom-left
		c = new Cell( cell.x, cell.y + cell.height * p2, cell.width * p1, cell.height * ( 1 - p2 ) )
		c.time = this.time++
		this.newCells.add( c )

		//bottom-right
		c = new Cell( cell.x + cell.width * p1, cell.y + cell.height * p2, cell.width * ( 1 - p1 ), cell.height * ( 1 - p2 ) )
		c.time = this.time++
		this.newCells.add( c )
	}
